// need to add if in moving foreware, if the class is apple then add one to the end in the opposite direction, also put all of it in try
class Grid {
    constructor() {
...
class Snake extends Grid {
    ...
    Movment() {
        console.log(snakePos)
        // 1. Get the current head coordinates and direction
        let currentX = snakePos[0][0];
        let currentY = snakePos[0][1];
        let currentDirection = snakePos[0][2];

        // 2. Calculate the NEW mathematical position (Math First)
        let newX = currentX;
        let newY = currentY;
        ...
        // 3. Update the array with the new head
        snakePos.unshift([newX, newY, currentDirection]);

        // 4. Draw the new head (Draw Second)
        // We use -1 because DOM rows and cells start at 0, but your game uses 1-based coordinates
        table.rows[newY - 1].cells[newX - 1].className = 'snake_current';

        // 5. Handle the tail (Hiding)
        // Pop the last element off the array and erase its color
